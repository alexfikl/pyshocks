...
@timeme
def evolve(scheme, solution, n: int, *, timestep: float,
        a: float = -1.0,
        b: float = 1.0,
        tfinal: float = 0.5,
        order: int = None,
...
@pytest.mark.parametrize(("scheme", "resolutions"), [
...
def test_burgers_convergence(scheme, resolutions, a=-1.0, b=1.0, tfinal=1.0):
    ...
    for n in resolutions:
        h_max, error = evolve(
                scheme, burgers.ex_shock, n, timestep=dt,
                a=a, b=b, tfinal=tfinal)
...
@pytest.mark.parametrize(("scheme", "order", "resolutions"), [
    (advection.Godunov(velocity=None), 1,
        list(range(80, 160 + 1, 16))),
    (advection.WENOJS32(velocity=None), 3,
        list(range(192, 384 + 1, 32))),
    (advection.WENOJS53(velocity=None), 5,
        list(range(32, 256 + 1, 32))),
    ])
def test_advection_convergence(scheme, order, resolutions,
        a=-1.0, b=+1, tfinal=1.0):
    ...
    def finalize(s, grid, quad):
        ...
        velocity = cell_average(quad,
                lambda x: 1.0 * continuity.velocity_const(grid, 0, x))
